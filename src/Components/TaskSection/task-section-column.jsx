...
const TaskSection= ({title, tasks, status, icon, handleDelete})=>{
    return(
        <>
        <section className="section">
            <h2 className='task__section_column'> <img src={icon} alt='' className='task__section_column-icon' /> {title}
            </h2>
            {tasks.map((task, index) => task.status === status &&  <TaskCard key={index} icon={BinCard} title={task.task} tags={task.tags} handleDelete={handleDelete} index={index} /> )}
        </section>
